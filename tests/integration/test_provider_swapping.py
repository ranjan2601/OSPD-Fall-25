...
def test_same_ai_different_chat_providers() -> None:
    ...
    with patch.object(discord_orch.chat_client, "get_message") as mock_get:
        mock_message = Mock()
    ...
    with patch.object(slack_orch.chat_client, "get_message") as mock_get:
        mock_message = Mock()
        mock_message.content = test_query
        mock_get.return_value = mock_message
...
def test_provider_independence() -> None:
    ...
    mock_discord.get_message.return_value = mock_discord_message
    mock_discord.send_message.return_value = True

    mock_slack = Mock()
    mock_slack_message = Mock()
    mock_slack_message.content = "Hello"
    mock_slack.get_message.return_value = mock_slack_message
