...
def channel_info(channel_id: str, access_token: str = Depends(require_user_token)) -> ChannelOut:
    client = _client_from_token(access_token)
    try:
...
def delete_message(
    ...
    if access_token == DUMMY_TOKEN:
        msgs = _SEEDED_MESSAGES.get(channel_id)
        if msgs is not None:
            _SEEDED_MESSAGES[channel_id] = [m for m in msgs if m.id != message_id and m.ts != message_id]
